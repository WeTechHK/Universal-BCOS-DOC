...
[Solidity](https://github.com/ethereum/solidity) is a high-level, statically typed, contract-oriented language for implementing smart contracts on the Ethereum platform. Although Solidity was originally designed for Ethereum, it is general enough to write smart contracts; therefore, it can also be used for other blockchain platforms, such as Universal BCOS.

Universal BCOS is officially compatible with **Cancun** Ethereum Virtual Machine (EVM) version. Please refer to [how to set the EVM version of solc](https://solidity.readthedocs.io/en/latest/using-the-compiler.html#setting-the-evm-version-to-target).  

Development tools such as [Remix](https://remix.ethereum.org/) \(a browser-based IDE\) and [Hardhat](https://hardhat.org/) \(a development framework\) can be utilized when developing smart contracts for Universal BCOS.
...
One way to compile Solidity code is to use the command-line compiler _solc_. This compiler can produce various outputs, ranging from simple binaries and assembly to an abstract syntax tree \(parse tree\). Assuming that the code above is saved in `HelloWorld.sol` \(`L03` is excluded in the source file shown above\), some examples of compiling the file `HelloWorld.sol` are as follows.
...
```bash
solc -o output --bin --ast --asm HelloWorld.sol
```
