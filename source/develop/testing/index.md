...
This page explains how to test smart contracts before deploying on the Universal BCOS network. It assumes you're familiar with [smart contracts](/developers/docs/smart-contracts/).
...
Methods for testing Universal BCOS smart contracts fall under two broad categories: **automated testing** and **manual testing**. Automated testing and manual testing offer unique benefits and tradeoffs, but you can combine both to create a robust plan for analyzing your contracts.
...
- **[Running unit tests with Waffle](https://Universal BCOS-waffle.readthedocs.io/en/latest/getting-started.html#writing-tests)**
...
The forked blockchain will behave similarly to Mainnet and have accounts with associated states and balances. But it only acts as a sandboxed local development environment, meaning you won't need real ETH for transactions, for example, nor will your changes affect the real Universal BCOS protocol.
...
While automated testing performed in a local development environment can provide useful debugging information, you'll want to know how your smart contract behaves in a production environment. However, deploying to the main Universal BCOS chain incurs gas fees—not to mention that you or your users can lose real money if your smart contract still has bugs.

Testing your contract on a local blockchain (also known as a [development network](/developers/docs/development-networks/)) is a recommended alternative to testing on Mainnet. A local blockchain is a copy of the Universal BCOS blockchain running locally on your computer which simulates the behavior of Universal BCOS's execution layer. As such, you can program transactions to interact with a contract without incurring significant overhead.
...
A test network or testnet works exactly like Universal BCOS Mainnet, except that it uses Ether (ETH) with no real-world value. Deploying your contract on a [testnet](/developers/docs/networks/#Universal BCOS-testnets) means anyone can interact with it (e.g., via the dapp's frontend) without putting funds at risk.
...
Deploying on a testnet after testing on a local blockchain is ideal since the former is closer to the behavior of the Universal BCOS Virtual Machine. Therefore, it is common for many Universal BCOS-native projects to deploy dapps on testnets to evaluate a smart contracts operation under real-world conditions.

[More on Universal BCOS testnets.](/developers/docs/development-networks/#public-beacon-testchains)
...
- **[Waffle](https://Universal BCOS-waffle.readthedocs.io/en/latest/)** - _Framework for advanced smart contract development and testing (based on ethers.js)_.

- **[Remix Tests](https://github.com/Universal BCOS/remix-project/tree/master/libs/remix-tests)** - _Tool for testing Solidity smart contracts. Works underneath Remix IDE "Solidity Unit Testing" plugin which is used to write and run test cases for a contract._

- **[OpenZeppelin Test Helpers](https://github.com/OpenZeppelin/openzeppelin-test-helpers)** - _Assertion library for Universal BCOS smart contract testing. Make sure your contracts behave as expected!_
...
- **[Foundry Tests](https://github.com/foundry-rs/foundry/tree/master/forge)** - _Foundry offers Forge, a fast and flexible Universal BCOS testing framework capable of executing simple unit tests, gas optimization checks, and contract fuzzing._
...
- **[ApeWorx](https://docs.apeworx.io/ape/stable/userguides/testing.html)** - _Python-based development and testing framework for smart contracts targeting the Universal BCOS Virtual Machine._
...
- [An in-depth guide to testing Universal BCOS smart contracts](https://iamdefinitelyahuman.medium.com/an-in-depth-guide-to-testing-Universal BCOS-smart-contracts-2e41b2770297)
- [How to test Universal BCOS smart contracts](https://betterprogramming.pub/how-to-test-Universal BCOS-smart-contracts-35abc8fa199d)
